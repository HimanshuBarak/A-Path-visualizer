...
class Spot:
	...
	def make_closed(self):
		if self.color !=PURPLE and self.color !=ORANGE:
		   self.color = RED
	...
	def update_neighbors(self, grid):
		...
		if (self.col < self.total_rows-1 and self.row >0)  and not grid[self.row-1][self.col + 1].is_barrier(): # lower left
			self.neighbors.append(grid[self.row-1][self.col + 1])
		if (self.col < self.total_rows-1 and self.row <self.total_rows-1)  and not grid[self.row+1][self.col + 1].is_barrier(): # lower left
			self.neighbors.append(grid[self.row+1][self.col + 1])			
